...
export const LinearGradient = (StyledLinearGradient: any) =>
  forwardRef(({ children, ...props }: any, ref: any) => {
    const { colors: colorsProp } = props;
    const colors = useThemeColors(colorsProp || []);
    return (
      <StyledLinearGradient ref={ref} {...props} colors={colors}>
        {children}
      </StyledLinearGradient>
    );
  });

// resolve gluestack ui theme values
...
const useThemeColors = (colors: string[]) => {
  const theme: any = nbConfig;
  return colors.map((color) => {
    if (color[0] === '$') {
      return theme.colors?.[color.slice(1)];
    }
    return color;
  });
};
